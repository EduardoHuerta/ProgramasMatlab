...
laba=A_lab(:,:,2); %Tomamos canal a (verdes y rojos)
z=zeros(size(labb));
z(laba<0 )=1; %máscara de tonos principalmente amarillos
...
new_a(z==1)=laba(z==1); %ponemos valores marcados de b en a
% new_b(z==1)=labb(z==1); %ponemos valores marcados de a en b
new_a(z==1)=new_a(z==1)*(-1); %Invertimos valores marcados de a

A_lab(:,:,3)=new_a; %Canal b modificado lo guardamos en imagen Lab
A_lab(:,:,2)=new_a;
A=lab2rgb(A_lab); %Conversión a RGB
